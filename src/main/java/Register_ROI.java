...
public class Register_ROI implements PlugInFilter
{
	...
	public void run(ImageProcessor ip)
    {
        ...
        try
        {
            ...
            Register reg = new RegisterFFT(a[0], width);
    ...
    protected void extractRegImages(ImagePlus imp)
    {
    ...
    protected float[][] registerRegImages(Register reg, float a[][])
    {
    	if (IJ.debugMode) {
            ImageStack is1 = BIJutil.imageStackFromMatrix(a, width);
            ImagePlus imp1 = new ImagePlus("regimages", is1);
            imp1.show();
        }
        ...
        float estimates[][] = new float[a.length - 1][];
        for (int j = 1; j < a.length; j++) {
            IJ.showProgress(j, a.length);
            IJ.showStatus("Registering " + j + "/" + a.length);
            estimates[j - 1] = reg.register(a[j]);
            ImageProcessor registered = Transformer.transform(imp.getStack().getProcessor(j + 1), estimates[j - 1]);
    ...
    protected void plotEstimates(float estimates[][])
    {
        float xValues[] = new float[estimates.length];
        for (int i = 0; i < xValues.length; i++) {
            xValues[i] = i + 1.0f;
        }

        float yValues[] = new float[estimates.length];
        for (int i = 0; i < yValues.length; i++)
            yValues[i] = estimates[i][0];

        PlotWindow pw = new PlotWindow("x-translations " + imp.getTitle(), "slice", "x-translation (pixels)", xValues, yValues);
        pw.draw();
        for (int i = 0; i < yValues.length; i++)
            yValues[i] = estimates[i][1];

        pw = new PlotWindow("y-translations " + imp.getTitle(), "slice", "y-translation (pixels)", xValues, yValues);
        pw.draw();
    }

    void showAbout()
    {
        IJ.showMessage("About this plugin...", "Registers a stack of images to the first slice using FFT cross-correlation");
    }
