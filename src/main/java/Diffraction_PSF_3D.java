...
public class Diffraction_PSF_3D implements PlugIn {
	...
	private static String[] norm = { "Peak = 1", "Peak = 255",
		"Sum of pixel values = 1" };
	// Constants for Bessel function approximation.
	private static double[] t = new double[] { 1, -2.2499997, 1.2656208,
		-0.3163866, 0.0444479, -0.0039444, 0.0002100 };
	private static double[] p = new double[] { -.78539816, -.04166397,
		-.00003954, 0.00262573, -.00054125, -.00029333, .00013558 };
	private static double[] f = new double[] { .79788456, -0.00000077,
		-.00552740, -.00009512, 0.00137237, -0.00072805, 0.00014476 };
	...
	public void run(final String arg) {
		...
		final GenericDialog gd = new GenericDialog("Specify psf", IJ.getInstance());
		...
		gd.addNumericField(
			"Longitudinal Spherical Aberration at max. aperture, same units", sa, 2);
		gd.addNumericField(
			"Image pixel spacing, same units (ccd cell spacing / magnification)",
			pixelSpacing, 2);
		...
		if (!IJ.showMessageWithCancel(
			...
			"Rayleigh resolution = " + IJ.d2s(dRing) + " pixels")) return;

		final float[][] pixels = new float[d][w * h];
		final int rMax = 2 + (int) Math.sqrt(ic * ic + jc * jc);
		final float[] integral = new float[rMax];
		final double upperLimit = Math.tan(Math.asin(na / indexRefr));
		final double waveNumber = 2 * Math.PI * indexRefr / lambda;
		// for (int k = 0; k <= d/2; k++){
		for (int k = 0; k < d; k++) {
			// int kSym = (2*kc - k) % d;
			// if(kSym == k){
			...
			for (int r = 0; r < rMax; r++) {
				final double kr = waveNumber * r * pixelSpacing;
				final int numCyclesJ = 1 + (int) (kr * upperLimit / 3);
				final int numCyclesCos =
					1 + (int) (Math.abs(kz) * 0.36 * upperLimit / 6);
				int numCycles = numCyclesJ;
				if (numCyclesCos > numCycles) numCycles = numCyclesCos;
				final int nStep = 2 * stepsPerCycle * numCycles;
				final int m = nStep / 2;
				final double step = upperLimit / nStep;
				double sumR = 0;
				double sumI = 0;
				// Simpson's rule
				...
				for (int j = 1; j < m; j++) {
					...
					kz =
						waveNumber *
							((k - kc) * sliceSpacing + sa * (u / upperLimit) *
								(u / upperLimit) * (u / upperLimit) * (u / upperLimit));
			...
			for (int j = 0; j < h; j++) {
				...
				for (int i = 0; i < w; i++) {
					final double rPixels =
						Math.sqrt((i - ic) * (i - ic) + (j - jc) * (j - jc));
		...
		if (normalization == PEAK1) {
			...
			for (int k = 0; k < d; k++) {
				for (int ind = 0; ind < n; ind++) {
					if (pixels[k][ind] > peak) peak = pixels[k][ind];
				}
			}
			final float f = 1 / peak;
			for (int k = 0; k < d; k++) {
				for (int ind = 0; ind < n; ind++) {
					pixels[k][ind] *= f;
				}
		...
		if (dB) {
			...
			for (int k = 0; k < d; k++) {
				for (int ind = 0; ind < n; ind++) {
					if (pixels[k][ind] > 0.000000001) pixels[k][ind] =
						(float) (SCALE * Math.log(pixels[k][ind]));
					else pixels[k][ind] = -90;
				}
			}
		}
	...
	float interp(final float[] y, final float x) {
		final int i = (int) x;
		final float fract = x - i;
	...
	double J0(final double xIn) {
		double x = xIn;
		if (x < 0) x *= -1;
		double r;
		if (x <= 3) {
			final double y = x * x / 9;
			r =
				t[0] + y *
					(t[1] + y * (t[2] + y * (t[3] + y * (t[4] + y * (t[5] + y * t[6])))));
		}
		else {
			final double y = 3 / x;
			final double theta0 =
			...
			final double f0 =
				f[0] + y *
					(f[1] + y * (f[2] + y * (f[3] + y * (f[4] + y * (f[5] + y * f[6])))));
