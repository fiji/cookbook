...
public class Zoomify_ implements PlugIn {
	...
	public void run(final String arg) {
		...
		final int iwidth = imp1.getWidth();
		final int iheight = imp1.getHeight();
		ip1.setInterpolate(true);
		int rzwidth, rzheight, rzx, rzy = 0;
		final Rectangle r = ip1.getRoi();
		...
		final GenericDialog gd = new GenericDialog("Zoom Movie");
		...
		final double maxScale2 = 1 / ((double) iheight / (double) height);
		if (maxScale > maxScale2) maxScale = maxScale2;
		final double scaleStep = (1 - maxScale) / (frames - 1);
		// double scales = maxScale;

		final ImageStack stackZoom = new ImageStack(width, height);
		...
		// while(scales<2)
		{
			...
			final double scales = maxScale + (z * scaleStep);
			IJ.showStatus("Zoomifying: " + z + "/" + frames + ".  ×" +
				IJ.d2s(scales * 100, 0) + "%. Esc to cancel");
			...
			rzx = (int) (x - ((double) rzwidth / (double) 2)) + (width / 2);
			rzy = (int) (y - ((double) rzheight / (double) 2)) + (height / 2);
			...
			xOffset = rzwidth / 2 - (width / 2);
			yOffset = (rzheight / 2) - (height / 2);
