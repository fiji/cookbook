...
public class Add_Scale_Bar_ implements PlugIn {
	...
	static final String[] locations = { "Upper Right", "Lower Right",
		"Lower Left", "Upper Left", "At Selection" };
	static final int UPPER_RIGHT = 0, LOWER_RIGHT = 1, LOWER_LEFT = 2,
			UPPER_LEFT = 3, AT_SELECTION = 4;
	static final String[] colors = { "White", "Black", "Light Gray", "Gray",
		"Dark Gray", "Red", "Green", "Blue", "Yellow" };
	...
	public void run(final String arg) {
		...
		if (imp != null) {
			if (showDialog(imp) && imp.getStackSize() > 1 && labelAll) labelSlices(imp);
		}
		else IJ.noImage();
	}

	void labelSlices(final ImagePlus imp) {
	...
	boolean showDialog(final ImagePlus imp) {
		...
		else if (location.equals(locations[AT_SELECTION])) location =
		...
		if (mag > 1.0) mag = 1.0;
		if (fontSize < (12 / mag)) fontSize = (int) (12 / mag);
		...
		if (roiX > 0 && roiWidth > 10) barWidth = roiWidth * pixelWidth;
			...
			if (barWidth > 0.67 * imageWidth) barWidth = 0.67 * imageWidth;
			if (barWidth > 5.0) barWidth = (int) barWidth;
		}
		...
		updateScalebar();
		final GenericDialog gd = new BarDialog("Scale Bar");
		...
		if (stackSize > 1) gd.addCheckbox("Label all Slices", labelAll);
		gd.showDialog();
		if (gd.wasCanceled()) {
		...
		if (stackSize > 1) labelAll = gd.getNextBoolean();
		return true;
	}

	void drawScaleBar(final ImagePlus imp) {
	...
	boolean updateLocation() {
		...
		final int fraction = 20;
		int x = width - width / fraction - barWidthInPixels;
		int y = 0;
		if (location.equals(locations[UPPER_RIGHT])) y = height / fraction;
		else if (location.equals(locations[LOWER_RIGHT])) y =
			height - height / fraction - barHeightInPixels - fontSize;
	...
	class BarDialog extends GenericDialog {
		...
		BarDialog(final String title) {
			super(title);
		}

		@Override
		public void textValueChanged(final TextEvent e) {
			final TextField widthField = ((TextField) numberField.elementAt(0));
			Double d = getValue(widthField.getText());
			if (d == null) return;
			barWidth = d.doubleValue();
			final TextField heightField = ((TextField) numberField.elementAt(1));
			d = getValue(heightField.getText());
			if (d == null) return;
			barHeightInPixels = (int) d.doubleValue();
			final TextField fontSizeField = ((TextField) numberField.elementAt(2));
			d = getValue(fontSizeField.getText());
			if (d == null) return;
			final int size = (int) d.doubleValue();
			if (size > 5) fontSize = size;
			updateScalebar();
		}

		@Override
		public void itemStateChanged(final ItemEvent e) {
			final Choice col = (Choice) (choice.elementAt(0));
			color = col.getSelectedItem();
			final Choice loc = (Choice) (choice.elementAt(1));
