...
public class Diffusion_ implements PlugInFilter
{
    ...
    public int setup(String arg, ImagePlus dummy)
    {
        if (arg.equals("about")) {
            showAbout();
            return DONE;
    ...
    public void run(ImageProcessor ip)
    {
        ...
        if (imp.getStackSize() == 1) {
            Diffusion df = new Diffusion(imp);
            if (df.params()) {
                df.compute();
                (new ImagePlus("Diffused ", df.getProcessor())).show();
            }
        } else {
            Diffusion3D df = new Diffusion3D();
            VolumeFloat v = new VolumeFloat(imp.getStack());
            if (df.params(imp, v)) {
                df.compute3D();
    ...
    void showAbout()
    {
        IJ.showMessage("About Diffusion plugin...", "Nonlinear diffusion filtering of images, and 3D and 4D volumes.\n(c) 1999-2003, Michael Abramoff\nhttp://bij.isi.uu.nl");
    }
}
